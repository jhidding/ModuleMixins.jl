...
# ╔═╡ 5ee13860-c55c-4a55-8671-679d8b06d172
using Unitful

# ╔═╡ ae821cd2-12d7-48de-a7fb-3b449e9a34e6
using CairoMakie

# ╔═╡ c205b4c3-5f92-47da-8e31-c168b59ad1ec
@kwdef struct Input
...
# ╔═╡ 5d870013-fa11-49ac-9ab0-be4ea00ae23f
...
# ╔═╡ ae40a317-fee2-4402-a6db-fb9610997168
function init(input::Input)
	State(
		time=0.0u"s",
...
# ╔═╡ cffe6d71-c208-408f-baf5-7ad1309af426
...
# ╔═╡ 329451fd-8e20-43bd-83aa-a571da969377
begin
	function step_time!(input::Input, state::State)
		Δt = input.time_step
		state.time += Δt / 2
	end
	
	function step_kick!(input::Input, state::State)
		Δt = input.time_step
		Δv = -state.position * input.spring_constant / input.weight * Δt
		state.velocity += Δv
	end
	
	function step_drift!(input::Input, state::State)
		Δt = input.time_step
		Δx = state.velocity * Δt
		state.position += Δx
	end
	
	function leap_frog_step!(input::Input, state::State)
		step_kick!(input, state)
		step_time!(input, state)
		step_drift!(input, state)
		step_time!(input, state)
	end
end

# ╔═╡ 92e0238a-2f95-49d1-8bce-5656a8a8650d
function run(input::Input)
	state = init(input)
	Channel{State}() do ch
		while state.time < input.time_end
			leap_frog_step!(input, state)
			put!(ch, deepcopy(state))
		end
	end
end

# ╔═╡ 079e6987-e81b-476b-99fc-9752e70814dd
function plot_result(output::Vector{State})
...
# ╔═╡ ac4d958f-03dd-49b6-8064-e2c88361452f
let
input = Input(
	time_step = 0.01u"s",
	time_end = 5.0u"s",
...
manifest_format = "2.0"
project_hash = "87b57381ea9802f5105cd324310c2537b20ed6da"
