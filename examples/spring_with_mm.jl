...
# ╔═╡ cd305b88-710c-11ef-1daa-93d73f0b6943
using Unitful

# ╔═╡ 5987439f-65ff-4afb-b38b-ef056b7a0337
using CairoMakie

# ╔═╡ 15de26f0-1344-470a-a59b-14fb9c005ff1
using ModuleMixins

# ╔═╡ 234f4e22-716b-4d88-94a3-0fd9c0242134
module Common
...
# ╔═╡ b6efb73f-ab2b-4edf-ac40-ed4f6f1644c9
@compose module Time
	using Unitful
	using ..Common
	...
	function run(model::Module, input::AbstractInput)
...
# ╔═╡ 1f9c6148-e069-43bd-8b62-291eb3afeebc
module LeapFrog
	using ..Common
	using ..Time

	function stepper(model::Module)
...
# ╔═╡ b2033c1d-06f6-4e3e-a3c0-5639f9b50f2e
@compose module Spring
	...
	step!(input::AbstractInput, state::AbstractState) = let a = accelleration(input, state)
		state.position += state.velocity * input.time_step
...
# ╔═╡ c848cd37-5dff-464e-8817-05b38cd61d25
@compose module LF_Spring
	@mixin Spring
	using ..LeapFrog

	Base.convert(::Type{State}, s::Spring.State) = State(time=s.time, velocity=s.velocity, position=s.position)

	const init = Spring.init
	const step! = LeapFrog.stepper(Spring)
end

# ╔═╡ 0ce1925d-e4cb-4660-b6c4-76a91eadfad0
function plot_result(output::Vector{T}) where {T <: Common.AbstractState}
...
# ╔═╡ 6d46a12e-710c-4b79-88bd-d8ddea4e5b01
let
input = Spring.Input(
	time_step = 0.01u"s",
	time_end = 5.0u"s",
...
output = Time.run(LF_Spring, input) |> collect
plot_result(output)
end
...
ModuleMixins = "2b5f0a26-aae2-5c4c-af55-bc6d20297121"
...
manifest_format = "2.0"
project_hash = "9408315fe9d8d892722c15ce5919bdb05253b876"
...
[[deps.ModuleMixins]]
deps = ["MacroTools"]
git-tree-sha1 = "31c64af39e29d2b46cbc771a230e6df77b5a4a52"
